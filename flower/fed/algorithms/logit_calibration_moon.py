...
class LogitCalibrationMoonContrastiveLearning:
  ...
  def compute_contrastive_loss(self, features: torch.Tensor, images: torch.Tensor) -> torch.Tensor:
    ...
    if torch.isnan(logits).any() or torch.isinf(logits).any():
      print("[FedLC-MOON] Warning: NaN/Inf detected in contrastive logits. Returning zero loss.")
      return torch.tensor(0.0, device=self.device, requires_grad=True)
    ...
    for epoch in range(epochs):
      ...
      for batch in train_loader:
        ...
        if torch.isnan(total_loss) or torch.isinf(total_loss):
          print(f"[FedLC-MOON] Warning: Loss became NaN/Inf. CE Loss={loss1.item():.4f}, Contrastive Loss={loss2.item():.4f}. Skipping batch.")
          continue
